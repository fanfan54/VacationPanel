        ...
        <nav class="navbar navbar-default navbar-fixed-top">
          <div class="container-fluid">
            ...
            <ul class="nav navbar-nav navbar-header navbar-right">
              ...
              <li class="divider-vertical"></li>
              <li><p class="navbar-text" id="user-resume"><span class="glyphicon glyphicon-user"></span>&nbsp;Bonjour <strong><?=$GLOBALS['loginApp']->textTruncate($_SESSION['user_firstname'],30) ?></strong><? if(in_array("worker",$_SESSION['user_role'])){?> <span data-toggle="tooltip" data-placement="bottom" title="Grade standard. Vous pouvez demander vos jours de congés, voir les jours de congés des autres employés si la fonction est activée dans cette entreprise, et gérer vos paramètres personnels." class="help-tooltip label label-default">employé</span><?} if(in_array("manager",$_SESSION['user_role'])){?> <span data-toggle="tooltip" data-placement="bottom" title="Grade supérieur. Destiné aux chefs de groupe et managers qui pourront gérer les demandes de congés des employés, les accepter ou les refuser, les contacter via l'application... Il peut gérer ses paramètres personnels et créer des comptes pour de nouveaux employés." class="help-tooltip label label-primary">gestionnaire</span><?} if(in_array("sysadmin",$_SESSION['user_role'])){?> <span data-toggle="tooltip" data-placement="bottom" title="Grade maximal. L'administrateur système est le responsable informatique chargé de la gestion de l'application. Il est en charge de mettre à jour l'application, s'occuper de son installation et créer des comptes gestionnaires. Il peux également effacer toute la base de données et gérer les paramètres globaux de l'application." class="label label-danger help-tooltip">admin</span><?} ?></p></li>
    ...
    ?>
      ...
      <div class="container">
        ...
        <div id="module-title" class="page-header col-md-9 col-xs-10">
		  ...
		  <? // Big bogue : le mode ne s'initalise pas correctement. Workaround TODO TODO : ne rien afficher par défaut et demander à l'utilisateur d'en choisir un à droite
		  ?>
                ...
                <div class="panel-body">
                  <div class="color-legend">
                    ...
                    <ul class="list-inline">
                      ...
                      <? if($GLOBALS['_setting_show_colleague_vacations']){?><li><span class="label label-aqua">&nbsp;</span> <label>Congé d'un collègue</label></li><?}?>
                    </ul>
                  </div>
                </div>
              </div>
    ...
    ?>
      ...
      <section class="container">
        ...
        <form accept-charset="utf-8" method="post" action="index.php?action=registerusers" name="registerform" class="col-md-8 col-md-offset-2 well">
          ...
          <fieldset>
            <div class="<? if($GLOBALS["register_input_username_haserror"] == true || $GLOBALS["register_input_username_pattern_haserror"] == true){echo "has-error ";}?>form-group">
              ...
              <div class="input-group">
                ...
                <input <? if($GLOBALS["register_input_haserror"] == false || $GLOBALS["register_input_username_haserror"] == true){echo "autofocus ";}?>required id="register_input_username" type="text" pattern="[a-z0-9\.]{2,64}" class="form-control input-lg" name="register_input_username" placeholder="Exemple : prenom.nom" />
                <span class="input-group-btn">
              ...
              <span class="<? if($GLOBALS["register_input_username_pattern_haserror"] == true){echo "pattern-error ";}?>help-block">Identifiant à retenir par l'utilisateur.<br>Lettres minuscules, chiffres et points "." seulement autorisés.<br>CONSEIL : Cliquez sur la loupe pour vérifier que le nom d'utilisateur choisi est disponible.<br>De 2 à 64 caractères au total.</span>
            </div>
            <div class="form-group">
              <label for="register_input_password_new">Mot de passe (obligatoire)</label>
              <div class="<? if($GLOBALS["register_input_password_mismatch"] == true || $GLOBALS["register_input_passwordnew_haserror"] == true || $GLOBALS["register_input_password_pattern_haserror"] == true){echo "has-error ";}?>input-group">
              ...
              <span class="<? if($GLOBALS["register_input_password_pattern_haserror"] == true){echo "pattern-error ";}?>help-block">Mot de passe confidentiel à retenir par l'utilisateur, il ne pourra pas être récupéré une fois perdu.<br>Au moins 6 caractères.</span>
            </div>
            <div class="form-group">
              <label for="register_input_password_repeat">Mot de passe (répétez) (obligatoire)</label>
              <div class="<? if($GLOBALS["register_input_password_mismatch"] == true || $GLOBALS["register_input_passwordrepeat_haserror"] == true){echo "has-error ";}?>input-group">
                <span onmouseenter="$('#register_input_password_new').attr('type','text');$('#register_input_password_repeat').attr('type','text');" onmouseleave="$('#register_input_password_repeat').attr('type','password');$('#register_input_password_new').attr('type','password');" class="input-group-addon input-lg">
                  <span class="glyphicon glyphicon-eye-open"></span>
                </span>
              ...
              <span class="<? if($GLOBALS["register_input_password_mismatch"] == true){echo "pattern-error ";}?>help-block">Afin de nous assurer que vous vous souviendrez du mot de passe, veuillez ré-entrer le même mot de passe une deuxième fois.</span>
            </div>
          </fieldset>
          <div class="<? if($GLOBALS["register_input_role_haserror"] == true){echo "has-error ";}?>form-group">
            <?
            if(in_array("manager",$_SESSION['user_role']) && in_array("sysadmin",$_SESSION['user_role'])) {
            ?>
            ...
            <select <? if($GLOBALS["register_input_role_haserror"] == true){echo "autofocus ";}?>required id="register_input_role" class="form-control input-lg" name="register_input_role">
              <option selected value="worker">Employé</option>
              <option value="manager">Gestionnaire</option>
              <option value="manager-worker">Gestionnaire et employé</option>
            </select>
            <span class="<? if($GLOBALS["register_input_role_haserror"] == true){echo "pattern-error ";}?>help-block">Le type de compte est très important, il détermine les autorisations de l'utilisateur sur le serveur.<br>
              <ul>
            ...
            <span class="<? if($GLOBALS["register_input_role_haserror"] == true){echo "pattern-error ";}?>help-block">Vous ne pouvez que choisir de créer un gestionnaire vu que vous n'êtes vous-même qu'un administrateur système. Vous pouvez cependant vous auto-proclamer gestionnaire dans vos Paramètres utilisateur <a id="openUserSettingsMenu" href="#">(cliquez ici pour ouvrir le menu)</a> afin de pouvoir créer un compte employé.</span>
            <?
            } elseif(in_array("manager",$_SESSION['user_role'])) {
            ?>
            ...
            <span class="<? if($GLOBALS["register_input_role_haserror"] == true){echo "pattern-error ";}?>help-block">Vous ne pouvez que choisir de créer un employé vu que vous n'êtes vous-même qu'un gestionnaire.</span>
            <?
            }
            ?>
		  ...
		  ?>
            ...
            <div class="<? if($GLOBALS["register_input_firstname_haserror"] == true || $GLOBALS["register_input_firstname_pattern_haserror"] == true){echo "has-error ";}?>form-group">
              <div class="input-group">
                <span class="input-group-btn">
                ...
                <input <? if($GLOBALS["register_input_firstname_haserror"] == true || $GLOBALS["register_input_firstname_pattern_haserror"] == true){echo "autofocus ";}?>required name="register_input_firstname" type="text" class="form-control input-lg" pattern=".{2,255}" id="register_input_firstname" placeholder="Prénom" />
              </div>
            </div>
            <div class="<? if($GLOBALS["register_input_lastname_haserror"] == true || $GLOBALS["register_input_lastname_pattern_haserror"] == true){echo "has-error ";}?>form-group">
              <div class="input-group">
                <label class="sr-only" for="register_input_lastname">Nom</label>
                <input <? if($GLOBALS["register_input_lastname_haserror"] == true || $GLOBALS["register_input_lastname_pattern_haserror"] == true){echo "autofocus ";}?>required name="register_input_lastname" type="text" class="form-control input-lg" pattern=".{2,255}" id="register_input_lastname" placeholder="Nom" />
                <span class="input-group-btn">
            ...
            <span class="<? if($GLOBALS["register_input_firstname_pattern_haserror"] == true || $GLOBALS["register_input_lastname_pattern_haserror"] == true){echo "pattern-error ";}?>help-block">Les noms et prénoms de l'utilisateur sont utilisés dans l'interface graphique (notamment dans les menus, les calendriers...) et dans les e-mailings à des fins de personnalisation.<br>CONSEIL : Cliquez sur l'éclair pour remplir automatiquement les champs si l'identifiant est au format prenom.nom.<br>Entre 2 et 255 caractères.</span>
          </div>
          <div class="<? if($GLOBALS["register_input_email_haserror"] == true || $GLOBALS["register_input_email_pattern_haserror"] == true){echo "has-error ";}?>form-group">
            ...
            <div class="input-group">
              ...
              <input <? if($GLOBALS["register_input_email_haserror"] == true || $GLOBALS["register_input_email_pattern_haserror"] == true){echo "autofocus ";}?>required id="register_input_email" type="email" class="form-control input-lg" name="register_input_email" placeholder="Cliquez sur l'éclair pour insérer l'identifiant" />
              <span class="input-group-btn">
          ...
          <div class="panel-group" id="accordion" role="tablist" aria-multiselectable="true">
            <div class="panel panel-default">
              ...
              <div id="collapseOne" class="panel-collapse collapse<? if($GLOBALS["register_input_days_pattern_haserror"] == true || $GLOBALS["register_input_defaulttab_haserror"] == true){echo ' in';}?>" role="tabpanel" aria-labelledby="headingOne">
                <div class="panel-body">
                  <?
            if(in_array("manager",$_SESSION['user_role'])) {
            ?>
                  ...
                  <div class="form-group" id="register_input_days_formgroup">
                    ...
                    <div class="<? if($GLOBALS["register_input_days_pattern_haserror"] == true){echo "has-error ";}?>input-group">
                      <input <? if($GLOBALS["register_input_days_pattern_haserror"] == true){echo "autofocus ";}?>id="register_input_days" type="number" class="form-control" name="register_input_days" min="0" value="25" />
                      <span class="input-group-btn">
                    ...
                    <span class="<? if($GLOBALS["register_input_days_pattern_haserror"] == true){echo "pattern-error ";}?>help-block">Détermine le solde de congés qu'aura à disposition l'employé dès la création de son compte.<br>L'employé pourra demander un nombre de jours de congés égal à son solde restant.</span>
                  </div>
            <?
            } 
            ?>
                  ...
                  <div id="register_input_defaulttab_formgroup" class="<? if($GLOBALS["register_input_defaulttab_haserror"] == true){echo "has-error ";}?>form-group">
            <?
            if(in_array("manager",$_SESSION['user_role']) && in_array("sysadmin",$_SESSION['user_role'])) {
            ?>
                    ...
                    <select disabled <? if($GLOBALS["register_input_defaulttab_haserror"] == true){echo "autofocus ";}?>required id="register_input_defaulttab" class="form-control input-lg" name="register_input_defaulttab">
          ...
          <div class="row form-group">
			...
			<? // Ici, big bogue : quand on reset le formulaire, bah le select de l'onglet par défaut n'est pas réinitialisé et reste sur une option sélectionnée (et des options désactivées) complètement incompztibles avec le nouveau type de compte. Workaround : on actualise la page
			?>
      ...
      ?>
            <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
              <ul class="nav nav-tabs" id="content-mode-tabs-nav" role="navigation">
                ...
                <? foreach($tabs_list as $temp_tabnav_data) {
                 ?>

                <li role="presentation"<? if($temp_tabnav_data[2]){?> class="active"<?}?>><a href="<?=$_SERVER['SCRIPT_NAME']."?action=".$temp_tabnav_data[1] ?>"><?=$temp_tabnav_data[0] ?></a></li>
                <?
                } ?>
